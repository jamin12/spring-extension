export const func = () => {
    ...
    for (const key in obj1) {
      if (obj1.hasOwnProperty(key)) {
        if (typeof obj1[key] === "object" && typeof obj2[key] === "object") {
          // 중첩된 객체인 경우 재귀적으로 비교
          result += `"${key}": {\n${highlightJsonDifferences(
            obj1[key],
            obj2[key]
          )}\n},\n`;
        } else if (obj1[key] !== obj2[key]) {
          // 값이 다를 경우 하이라이트 처리
          result += `<span style="background-color: #ffcccc;">"${key}": ${JSON.stringify(
            obj1[key]
          )}</span>,\n`;
        } else {
          // 값이 같을 경우 그대로 출력
  ...
  async function saveAPIToLocalStorage() {
    ...
    try {
      ...
      console.error(
        "네트워크창에서 api doc을 읽어오는 url을 등록해주세요:",
        error
      );
      alert("네트워크창에서 api doc을 읽어오는 url을 등록해주세요");
    }
        ...
        const responses = details.responses
          ? Object.entries(details.responses).reduce(
              (acc, [statusCode, response]) => {
                ...
                acc[statusCode] = resolveRef(
                  _response.content?.["*/*"]?.schema?.items
                    ? _response.content?.["*/*"]?.schema?.items
                    : _response.content?.["*/*"]?.schema,
                  apiDoc
                );
                return acc;
              },
  ...
  async function compareAndHighlightChanges() {
    try {
      ...
      document.querySelectorAll(".opblock").forEach((element, index) => {
        ...
        if (apiId_path === null) {
          apiId = element
            .querySelector(".opblock-summary-path__deprecated")
            .textContent.trim();
        } else {
        ...
        if (newAPIs[apiKey]) {
          ...
          if (storedAPIs[apiKey]) {
            ...
            if (
              ...
              (element as any).style.border = "2px solid red"; // 변경된 API 항목에 빨간 테두리 적용
              addIndicator(index, apiId, method, "modified"); // 인디케이터 추가
              addUpdateButton(element, apiKey, newAPIs[apiKey]); // 업데이트 버튼 추가
            }
          } else {
            (element as any).style.border = "2px solid red"; // 새로운 API 항목에 빨간 테두리 적용
